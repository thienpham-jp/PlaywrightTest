...
// Reads the machine's private (local network) IPv4 address so rate-limit
// logs can be correlated with the source IP being throttled.
...
test.describe("Rate Limit Auth API", () => {
  ...
  /** Test Cases for Rate Limit Auth API method `GET /publishers/auth/{username}?isGlobal=&countryCode=`
  ...
  test("TC_01 - Verify API basic call 1 time - Expect 200 OK", async ({
    ...
    const clientIp = getPrivateIp();
    console.log(`Client private IP address : ${clientIp}`);
  ...
  test.skip("TC_02 - Send 31 requests within 1 minute - Expect rate limit (429) triggered", async ({
    ...
    const clientIpAfter = getPrivateIp();

    console.log(`Client private IP address : ${clientIp}`);
    console.log(`Client private IP address (after): ${clientIpAfter}`);
  ...
  test.skip("TC_04 - Send requests continuously until rate limit (429) is triggered - Log statistics", async ({
    ...
    // Capture the client IP once and reuse it for the whole run so we know
    // every request in this test is guaranteed to originate from the same IP.
    const clientIp = getPrivateIp();
    ...
    // Re-check the IP after the run to confirm it stayed the same throughout
    // (guards against NAT/network changes invalidating the per-IP rate limit test).
    const clientIpAfter = getPrivateIp();
    ...
    console.log(`Client private IP address (before) : ${clientIp}`);
    console.log(`Client private IP address (after)  : ${clientIpAfter}`);
    ...
    // All requests must have been sent from the same IP address for the
    // rate-limit result to be valid.
    if (clientIp !== "unknown" && clientIpAfter !== "unknown") {
      expect(clientIpAfter).toBe(clientIp);
    }
